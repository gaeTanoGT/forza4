...
public class FrameGriglia extends Interfaccia implements Runnable{
    ...
    public void butBack() {
    	//TODO: va resettato DataFile
    	System.err.println("BBBBAAAAcKKKKK");
    	mostraFrame(false);
    	
    	DataFile d = new DataFile();
		//TODO: deve togliere 1 da numero partite
    	d.removeMatch();
    	//d.updateData(-1);
    ...
    protected boolean verificaEnd(Boolean pl, DataFile file, boolean bot, int liv) throws InterruptedException {
		...
		if(tipologia == 0) {
			...
			try {
				if(riempiCol(col, pl)) {		//se assegnazione è avvenuta con successo
					if(!verificaEnd(pl, file, false, -1))
						pl = !pl;	
				}
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
			...
			if(!pl) {
				...
				try {
					if(riempiColBot(col, pl)) {
						if(!verificaEnd(pl, file, false, liv))
							pl = !pl;
					}
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
			    ...
			    try {
					if(!verificaEnd(pl, file, true, liv))
						pl = !pl;
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
